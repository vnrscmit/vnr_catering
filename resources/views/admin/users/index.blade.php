    ...
    <!-- Custom CSS for checkbox and buttons -->
    ...
    <script type="text/javascript">
        $(function() {
            ...
            function updateSelectedCount() {
                ...
                var hasSelected = selected > 0;
                $('#send-email-btn, #update-start-date-btn').prop('disabled', !hasSelected);
            }
            ...
            $(document).on('click', '#send-email-btn', function() {
                ...
                $('#selected-user-ids').val(selectedIds.join(','));
            });

            // Handle Update Start Date button
            $(document).on('click', '#update-start-date-btn', function() {
                var selectedIds = getSelectedIds();
                if (selectedIds.length === 0) {
                    alert('Please select at least one user.');
                    return;
                ...
                // Open update start date modal
                $('#updateStartDateModal').modal('show');
                // Populate hidden input with selected IDs
                $('#selected-user-ids-date').val(selectedIds.join(','));
            });
            ...
            // On modal close, reset selection
            $('#sendEmailModal, #updateStartDateModal').on('hidden.bs.modal', function() {
                resetSelection();
            });
        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="">
                        <button type="button" id="send-email-btn" class="btn btn-info btn-sm" disabled>
                            <i class="fa fa-envelope"></i> Send Email
                        </button>
                        <button type="button" id="update-start-date-btn" class="btn btn-warning btn-sm" disabled>
            ...
            <div class="modal fade" id="sendEmailModal" tabindex="-1" aria-labelledby="sendEmailModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form action="{{ route('admin.users.emailBulk') }}" method="POST" id="sendEmailForm">
                            @csrf
                            <input type="hidden" name="user_ids" id="selected-user-ids">

                            <div class="modal-header">
                                <h5 class="modal-title" id="sendEmailModalLabel">
                                    <i class="fa fa-envelope text-info"></i> Confirm Email Sending
                                </h5>
                            ...
                            <div class="modal-body">
                                <div class="text-center">
                                    ...
                                    <p class="text-muted mb-0">
                                        You are about to send an email to the selected users.
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary" id="confirmSendBtn">
                                    <i class="fa fa-paper-plane"></i> Yes, Send Email
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <div class="modal fade" id="updateStartDateModal" tabindex="-1">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form action="{{ route('admin.users.updateDateBulkNew') }}" method="POST">
                            @csrf
                            <input type="hidden" name="user_ids" id="selected-user-ids-date">
                            <div class="modal-header">
                                <h5 class="modal-title">
                                    <i class="fa fa-calendar"></i> Update Start Date
                                </h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body">
                                ...
                                <div class="alert alert-info">
                                    <i class="fa fa-info-circle"></i> This will update the start date for all selected users.
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                                <button type="submit" class="btn btn-primary">
                                    <i class="fa fa-calendar-check-o"></i> Update Start Date
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <form action="{{ route('admin.users.suspend') }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="mb-3">
                                    ...
                                    <input type="date" name="suspend_date" class="form-control" value="{{ date('Y-m-d') }}" readonly required>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">
                                        Reason <span class="text-danger">*</span>
                                    </label>
                                    <textarea name="suspend_remarks" class="form-control" rows="4" placeholder="Enter Inactive reason..." required></textarea>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
