            ...
            <div class="card-body">
                <form action="{{ route('locations.update', $location->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('short_code') is-invalid @enderror" id="short_code" name="short_code" value="{{ old('short_code', $location->short_code) }}" required>
                        @error('short_code')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 form-check">
                        <input type="checkbox" class="form-check-input" id="status" name="status" value="1" {{ $location->status === 1 ? 'checked' : '' }}>
